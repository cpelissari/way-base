...
public abstract class TarefaAbstrata<T> implements Tarefa<T> {
  ...
  public T call() throws Exception {
    try {
      T resultado = executar();

      agenda.marcarTerminado(agendamento);

      return resultado;
    } catch (Exception e) {
      e.printStackTrace();
      agenda.sinalizarErro(agendamento);
      throw e;
    } finally {
      finalizar();
    }
  ...
  protected Identificador<T> getIdentificador() {
    return agendamento.getIdentificador();
  }

  protected <C extends Configuracao> C getConfiguracao() {
    return agendamento.<C> getConfiguracao();
  }
